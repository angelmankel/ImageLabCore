...
- **What models are installed, and what are they?** `/imagelab/hashes` returns a SHA-256 index of
  ...
  from.
- **Fetch this model.** `/imagelab/downloads` starts a CivitAI download, reports progress, and
  cancels. `/imagelab/models/…` deletes one again.
- **Keep this picture.** `/imagelab/favorites` copies a generation out of ComfyUI's `temp/`, which
...
| Method | Path | |
|---|---|---|
| GET | `/imagelab/hashes` | model hash index; send `If-None-Match` to get a 304 |
| GET | `/imagelab/downloads` | every tracked download, active or finished |
| POST | `/imagelab/downloads` | start one — `{version_id, folder?, filename?}` |
| DELETE | `/imagelab/downloads/{version_id}` | cancel, or dismiss a finished row |
| DELETE | `/imagelab/models/{type}/{filename}` | delete a local model, drop it from the index |
| GET | `/imagelab/favorites` | list; ETag'd |
| POST | `/imagelab/favorites` | save `{filename, subfolder?, type?}` out of temp/ |
| DELETE | `/imagelab/favorites/{date}/{filename}` | remove one |
| GET | `/imagelab/favorites/view` | the image bytes for one favorite |
